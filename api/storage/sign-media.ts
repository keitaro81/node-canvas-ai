...
// L2 テナント分離: 署名は service role の本エンドポイントのみが行う（0009 で blanket SELECT を drop）。
// 呼び出し者 JWT のアクセス権を検証してからのみ署名する。
// 3つの入力（混在可）:
//   { workflowId }  … そのワークフローにアクセス可能なら canvas_data+thumbnail の全メディアを署名（loadWorkflow）
//   { urls }        … 各 url は generations.output_url。所有 or その WF が public/team 共有なら署名（History/サムネ）
//   { ownUrls }     … storage.objects.owner が呼び出し者本人のオブジェクトのみ署名（生成/参照アップロード直後の即時表示）
// 返り値: { map: { 入力canonicalURL: 署名URL, ... } }（不可は省略）
...
async function authorizeGenerationUrls(admin: any, urls: string[], userId: string, callerTeamId: string | null): Promise<string[]> {
  ...
  const { data: gens } = await admin
    .from('generations')
    .select('output_url, user_id, workflow_id')
    .in('output_url', urls)
  ...
  if (needWf.length) {
    ...
    const { data: wfs } = await admin
      .from('workflows').select('id, visibility, team_id').in('id', wfIds)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
